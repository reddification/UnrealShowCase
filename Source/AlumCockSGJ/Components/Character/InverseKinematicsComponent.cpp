...
{
	Super::BeginPlay();
	checkf(IsValid(IkSettings), TEXT("IK Settings DataAsset must be set"));
	DebugDrawType = IkSettings->bDrawDebug ? EDrawDebugTrace::ForOneFrame : EDrawDebugTrace::None;
}
	...
	const FVector& ActorLocation, float CapsuleHalfHeight, bool bCrouched)
{
	...
	if (bHit)
	{
		float Elevation = (HitResult.Location.Z - (ActorLocation.Z - CapsuleHalfHeight)) / IkData.IKScale;
	...
	const FName& ToesSocketName, float PreviousValue)
{
	...
	const float MaxFootOffset = IkSettings->FootLength;
	bool bHeelHit = UKismetSystemLibrary::LineTraceSingle(this, HeelLocation,
		HeelLocation - FVector::UpVector * IkSettings->TraceExtend, TraceType,true,TArray<AActor*>(),
		DebugDrawType, HitResult, true);
	float HeelDistance = bHeelHit ? FMath::Min(HitResult.Location.Z - HeelLocation.Z, MaxFootOffset) : 0.f;

	bool bToesHit = UKismetSystemLibrary::LineTraceSingle(this, ToesLocation,
		ToesLocation - FVector::UpVector * IkSettings->TraceExtend, TraceType,true,TArray<AActor*>(),
		DebugDrawType, HitResult, true);
	float ToesDistance = bToesHit ? FMath::Min(HitResult.Location.Z - ToesLocation.Z, MaxFootOffset) : 0.f;

	bool bFootHit = UKismetSystemLibrary::LineTraceSingle(this, FootLocation,
		FootLocation - FVector::UpVector * IkSettings->TraceExtend, TraceType,true,TArray<AActor*>(),
		DebugDrawType, HitResult, true);
	float FootDistance = bFootHit ? FMath::Min(HitResult.Location.Z - FootLocation.Z, MaxFootOffset) : 0.f;
