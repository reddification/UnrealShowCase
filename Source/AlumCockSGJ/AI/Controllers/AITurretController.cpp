...
AAITurretController::AAITurretController(const FObjectInitializer& ObjectInitializer)
...
void AAITurretController::Think()
{
	...
	{
		ABaseHumanoidCharacter* PerceivedCharacter = Cast<ABaseHumanoidCharacter>(PerceivedActor);
		if (IsValid(PerceivedCharacter))
		{
			if (!PerceivedCharacter->GetBaseCharacterAttributesComponent()->IsAlive())
			{
				continue;
			}
		}
